...
const SubmitButton = ({ processing, error, children, disabled }) => (
    <button
        className={`btn ${error ? "btn-outline-danger" : "btn-outline-warning"}`}
        type="submit"
...
function CheckoutForm (props) {
    ...
    const handleSubmit = async (event) => {
        ...
        if (payload.error) {
            ...
            console.log(body);
            API.post("reservations", body).then(res=>{
                console.log(res);
            }).catch(err=>{
                console.log(err);
            })
    ...
    return paymentMethod ? (
        <div className="Result">
            <div className="ResultTitle" role="alert">
                Paiement réussi. Vous allez recevoir un ticket par email dans les prochaines minutes.
            </div>
            {/* <div className="ResultMessage mt-3 text-danger">
                Thanks for trying Stripe Elements. No money was charged, but we
                generated a PaymentMethod: {paymentMethod.id}
            </div>
            <ResetButton onClick={reset} /> */}
            <button className="btn btn-outline-warning mt-3" onClick={()=>history.push("/")}>Retourner à l'accueil</button>
        </div>
    ) : (
